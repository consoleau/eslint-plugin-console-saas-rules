  ...
  create: (context) => {
    return {
      'ClassDeclaration': (node) => {
        // class has no methods/constructor, so nothing to check...
        ...
        constructor.value.params.forEach(param => {
          assignmentsToThis.forEach( assignment => {
            if (param.name !== undefined && assignment.expression.right.name === param.name && assignment.expression.left.property.name !== param.name) {
